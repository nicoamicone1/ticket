...
-- IMPORTANTE: calificar storage.objects.name. Si se usa solo "name" dentro del
-- subquery "FROM spaces s", Postgres lo resuelve a spaces.name (la columna del
-- espacio) en vez del nombre del archivo, y la política SIEMPRE rechaza.
-- La ruta es {space_id}/{ticket_id}/{ts}_{nombre} → foldername[1] = space_id.

-- Limpiar cualquier nombre de política usado en intentos previos
...
-- DELETE: necesario para limpiar los archivos al eliminar un ticket
DROP POLICY IF EXISTS "Borrar adjuntos de espacios propios" ON storage.objects;
CREATE POLICY "Borrar adjuntos de espacios propios"
